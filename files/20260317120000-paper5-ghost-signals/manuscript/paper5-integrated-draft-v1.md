...
However, first-detection dates are statistically extreme values, inherently sensitive to population size (Miller-Rushing et al., 2008; Moussus et al., 2010; Roth et al., 2014). When populations decline, detection probability decreases, producing apparent delays — "ghost signals" — defined operationally as statistically significant delays in first-detection dates that (a) contradict the direction expected from concurrent temperature trends and (b) correlate with independent evidence of population decline. This detection-bias mechanism has been theoretically predicted (Miller-Rushing et al., 2008) and demonstrated for individual species (Roth et al., 2014, who showed that declining Wood Thrush populations produced apparent phenological delays), but has not been validated empirically at multi-taxon, multi-decadal scale.
...
Six alternative hypotheses were evaluated using independent statistical tests. Each represents a distinct scientific question regarding the mechanism of delay; therefore, no family-wise error rate correction was applied in the main text (Rothman, 1990). However, for transparency, Benjamini-Hochberg FDR-corrected p-values are reported in Supplementary Table S3, confirming that the pattern of significance is robust to multiple testing adjustment. Tests and predictions are detailed in Table 1.
...
First-calling dates delayed by +5.3 days decade⁻¹ (p < 0.0001, n = 41 stations, 67 years). Delays were detected at 34/41 stations (83%). Over the full observation period, this represents a cumulative delay of ~37 days. Spring cicada (*T. nigricosta*) delayed by +2.0 days decade⁻¹ (p = 0.04, n = 41 stations). Year-to-year temperature variation showed the expected negative correlation with first-calling date (warmer → earlier, r = −0.20), confirming intact temperature sensitivity.
...
- **Fig. 2**: Japanese tree frog first-calling date time series (1953–2020) with linear trend (+5.3 days decade⁻¹, station-level mean of 41 stations). Note: the annual-aggregate regression slope is +3.1 days decade⁻¹, reflecting compositional changes in reporting stations over time; the station-level mean is used throughout as the primary estimate.
